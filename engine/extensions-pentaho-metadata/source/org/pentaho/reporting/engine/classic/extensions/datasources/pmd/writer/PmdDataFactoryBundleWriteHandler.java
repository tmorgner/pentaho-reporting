...
public class PmdDataFactoryBundleWriteHandler implements BundleDataFactoryWriterHandler
{
  ...
  /**
   * Writes a data-source into a own file. The name of file inside the bundle is returned as string. The file name returned is always absolute and can be made
   * relative by using the IOUtils of LibBase. If the writer-handler did not generate a file on its own, it should return null.
   *
      ...
      BundleWriterException
  {
    ...
    configAttrs.setAttribute(PmdDataFactoryModule.NAMESPACE, "label-mapping", String.valueOf(df.isLabelMapping()));
    ...
    for (int i = 0; i < queryNames.length; i++)
    {
          ...
          (StringUtils.isEmpty(queryScriptLanguage) == false || StringUtils.isEmpty(globalScriptLanguage) == false))
      {
        ...
        {
          xmlWriter.writeTag(PmdDataFactoryModule.NAMESPACE, "script", "language", queryScriptLanguage, XmlWriterSupport.OPEN);
        }
