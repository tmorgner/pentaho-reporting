...
public class PmdMetaTableModel implements MetaTableModel, CloseableTableModel
{
                           ...
                           final List<Selection> selections)
  {
    ...
    for (int i = 0; i < this.selections.length; i++) {
      ...
      if (uniqueIds.contains(columnName)) {
        ...
        columnName += ((this.selections[i].getAggregationType() != null) ? ":" + this.selections[i].getAggregationType().toString() : "");
      }
  ...
  {
    return new PentahoMetaDataAttributes(parentTableModel.getCellDataAttributes(row, column), selections[column].getLogicalColumn());
  }
  ...
  {
    return new PentahoMetaDataAttributes(parentTableModel.getColumnAttributes(column), selections[column].getLogicalColumn());
  }
