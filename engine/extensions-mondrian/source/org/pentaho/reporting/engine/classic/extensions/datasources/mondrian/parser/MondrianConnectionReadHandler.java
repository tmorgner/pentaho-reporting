...
public class MondrianConnectionReadHandler extends AbstractXmlReadHandler
{
  ...
  private String readFiltered(final Attributes attrs, final String attr)
  {
    final String value = attrs.getValue(getUri(), attr);
  ...
  protected void startParsing(final Attributes attrs) throws SAXException
  {
    ...
    dataSourceName = readFiltered(attrs, "datasource-name");
    connectionString = readFiltered(attrs, "connection-string");
    driverClass = readFiltered(attrs, "jdbc-driver");
    mondrianCubeDefinitionFile = readFiltered(attrs, "mondrian-cube-definition");

    designTimeName = readFiltered(attrs, "design-time-name");

    jdbcPassword = PasswordEncryptionService.getInstance().decrypt
        (getRootHandler(), readFiltered(attrs, "jdbc-password"));
    jdbcPasswordField = readFiltered(attrs, "jdbc-password-field");

    jdbcUser = readFiltered(attrs, "jdbc-user");
    jdbcUserField = readFiltered(attrs, "jdbc-user-field");

    role = readFiltered(attrs, "role");
    roleField = readFiltered(attrs, "role-field");
  }
