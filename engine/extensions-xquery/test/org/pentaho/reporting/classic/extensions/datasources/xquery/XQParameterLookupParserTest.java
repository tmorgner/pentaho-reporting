...
public class XQParameterLookupParserTest extends TestCase
{
  ...
  public void testConventionalExternalDeclaration()
  {
    ...
    assertEquals("No fields should have been detected", 0, parser.getFields().length);
  ...
  public void testReportingExternalDeclaration()
  {
    ...
    assertEquals("report.name property not found", 1, parser.getFields().length);
    assertEquals("report.name property not saved", "report.name", parser.getFields()[0]);
