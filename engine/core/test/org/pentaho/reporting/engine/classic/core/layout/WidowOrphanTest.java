...
//    ModelPrinter.INSTANCE.print(logicalPageBox3);
  ...
  /**
   * Widows override keep-together, and widow declarations and the widow-blockout-area do not take
   * parent keep-together areas into account. A orphan combines with keep-together, but if conflicting,
   * the orphan rule will win.
   *
  ...
  /**
   * Tests the combined effect of widow, orphan and keep-together rules.
   * Orphan rules trump keep-together declarations for elements included in the orphan calculation.
   * A keep-together child will expand the orphan area and thus triggers an early orphan-pagebreak.
   * After that break the band stays with the orphans.
   *
   * Keep-together wins over widow rules to the effect that the keep-together area expands the
   * widow area.
   *
   * @throws Exception
   */
  ...
  /**
   * Same as above with an additional padding band added. This ensures that itembands for
   * testing do not naturally fall on a pagebreak.
   *
   * @throws Exception
   */
  ...
  private Band createBand(final String name, final float height)
  {
    ...
    ghO1.addElement(WidowTest.createDataItem(name, 100, height));
    return ghO1;
  }
}
