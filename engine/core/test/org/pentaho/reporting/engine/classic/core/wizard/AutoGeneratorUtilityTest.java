...
public class AutoGeneratorUtilityTest extends TestCase
{
  ...
  public void testComputeFieldWidths()
  {
    ...
    allRelative[0] = new Float(-20);
    allRelative[1] = new Float(-20);
    allRelative[2] = new Float(-20);
    allRelative[3] = new Float(-20);
    allRelative[4] = new Float(-20);
    final float[] result2 = AutoGeneratorUtility.computeFieldWidths(allRelative, 500);
    for (int i = 0; i < result2.length; i++)
    {
      float aFloat = result2[i];
      assertEquals(-20, aFloat, 0);
    }

    final Float[] allStatic = new Float[5];
    allStatic[0] = new Float(100);
    allStatic[1] = new Float(100);
    allStatic[2] = new Float(100);
    allStatic[3] = new Float(100);
    allStatic[4] = new Float(100);
    final float[] result3 = AutoGeneratorUtility.computeFieldWidths(allStatic, 500);
    for (int i = 0; i < result3.length; i++)
    {
      float aFloat = result3[i];
      assertEquals(-20, aFloat, 0);
    }

    final Float[] allMixed = new Float[5];
    allMixed[0] = new Float(100);
    allMixed[1] = new Float(100);
    allMixed[2] = new Float(-20);
    allMixed[3] = new Float(-20);
    allMixed[4] = null;
    final float[] result4 = AutoGeneratorUtility.computeFieldWidths(allMixed, 500);
    for (int i = 0; i < result4.length; i++)
    {
      float aFloat = result4[i];
      assertEquals(-20, aFloat, 0);
    }
