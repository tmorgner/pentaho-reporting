...
public class Prd4661Test extends TestCase
{
  public Prd4661Test()
  {
  }
  ...
  public void testBorderExpandsTotalSize_Canvas() throws Exception
  {
    ...
    itemBand.getStyle().setStyleProperty(ElementStyleKeys.BORDER_TOP_WIDTH, 40f);
    final Element element = (Element) itemBand.getElement(0);
    element.getStyle().setStyleProperty(ElementStyleKeys.POS_X, 0f);
    element.getStyle().setStyleProperty(ElementStyleKeys.POS_Y, 0f);

    // in 3.9, a band that only contains percentage-height elements, will resolve to a height of 0.
    final Element derive = element.derive();
    derive.getStyle().setStyleProperty(ElementStyleKeys.POS_X, 100f);
    derive.getStyle().setStyleProperty(ElementStyleKeys.MIN_HEIGHT, 10f);
    itemBand.addElement(derive);

    itemBand.setLayout(BandStyleKeys.LAYOUT_CANVAS);
    itemBand.setName("itemband");

    final LogicalPageBox box = DebugReportRunner.layoutSingleBand(report, itemBand, false, false);
    Assert.assertEquals(5000000, box.getLastChild().getY2());
    Assert.assertEquals(5000000, box.getHeight());
