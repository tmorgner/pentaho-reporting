...
public class LegacyUpdateStructureVisitor extends AbstractStructureVisitor
{
  ...
  public void inspect(final AbstractReportDefinition report)
  {
    ...
    {
      Band parent = report.getParent();
      if (parent == null)
      {
        return;
      }

      ReportDefinition reportDefinition = parent.getReportDefinition();
      while (reportDefinition != null && reportDefinition instanceof MasterReport == false)
      {
        final Band reportParent = report.getParent();
        if (reportParent == null)
        {
          return;
        }
        reportDefinition = reportParent.getReportDefinition();
      }
                                            ...
                                            final ExpressionMetaData expressionMetaData)
  {
    ...
    {
      LegacyUpdateHandler handler = (LegacyUpdateHandler) expression;
