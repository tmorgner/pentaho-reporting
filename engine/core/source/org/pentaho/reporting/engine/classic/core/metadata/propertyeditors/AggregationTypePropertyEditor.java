...
public class AggregationTypePropertyEditor implements PropertyEditor
{
  ...
  public String getAsText()
  {
    ...
    {
      if (ObjectUtilities.equal(availableTypes[i], value))
      {
        return availableTypeNames[i];
      }
    }
    return null;
  }
  ...
  public void setAsText(final String text) throws IllegalArgumentException
  {
    ...
    for (int i = 0; i < availableTypeNames.length; i++)
    {
      if (ObjectUtilities.equal(availableTypeNames[i], text))
      {
        setValue(availableTypes[i]);
        return;
      }
    }
  ...
  {
    return (String[]) availableTypeNames.clone();
  }
