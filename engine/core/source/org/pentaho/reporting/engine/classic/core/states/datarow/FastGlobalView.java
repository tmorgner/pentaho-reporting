...
public final class FastGlobalView implements DataRow
{
  ...
  public Object get(final String col) throws IllegalStateException
  {
    ...
    if (idx < 0)
    {
      if (warnInvalidColumns)
      {
        ...
        {
          FastGlobalView.logger.warn("Warning: Data-Set does not contain a column with name '" + col + '\'');
        }
      }
      return null;
    }
  ...
  public boolean isChanged(final String name)
  {
    ...
    if (idx < 0)
    {
      if (warnInvalidColumns)
      {
        ...
        {
          FastGlobalView.logger.warn("Warning: Data-Set does not contain a column with name '" + name + '\'');
        }
      }
      return false;
    }
  ...
  public void removeColumn(final String name)
  {
    ...
    if (FastGlobalView.DEBUG)
    {
      FastGlobalView.logger.debug(
    ...
    if (FastGlobalView.DEBUG)
    {
      FastGlobalView.logger.warn("Out of order removeal of a column: " + name);
    }
    ...
    if (FastGlobalView.DEBUG)
    {
      FastGlobalView.logger.debug("New Namecache: " + nameCache);
    }
  }
                       ...
                       final boolean update)
  {
    if (FastGlobalView.DEBUG)
    {
      if (update)
      {
        FastGlobalView.logger.debug("  +   : " + name);
      }
      else
      {
        FastGlobalView.logger.debug("Adding: " + name);
      }
  ...
  private void updateColumn(final String name, final Object value)
  {
    final Integer o = (Integer) nameCache.get(name);
    if (o == null)
    {
