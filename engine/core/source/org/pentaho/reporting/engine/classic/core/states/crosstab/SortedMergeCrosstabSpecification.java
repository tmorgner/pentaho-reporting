...
public class SortedMergeCrosstabSpecification implements CrosstabSpecification
{
  ...
  public SortedMergeCrosstabSpecification(final ReportStateKey key, final String[] columnSet)
  {
    if (key == null)
    {
  ...
  public void add(final DataRow dataRow)
  {
    ...
    while (entries.size() > insertationCursor)
    {
      final Object[] existingKey = (Object[]) entries.get(insertationCursor);
      final int result = comparator.compare(newKey, existingKey);
      if (result == 0)
      {
        // do not add, as the entry already exists.
        return;
      }
      if (result < 0)
      {
