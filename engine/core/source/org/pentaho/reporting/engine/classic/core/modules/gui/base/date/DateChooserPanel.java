...
public class DateChooserPanel extends JPanel
{
  ...
  private class YearSelectionAction implements ActionListener
  {
    ...
    public void actionPerformed(final ActionEvent e)
    {
      if (!refreshing)
      {
        ...
        final int dayOfMonth = dateView.get(Calendar.DAY_OF_MONTH);
        dateView.set(Calendar.DAY_OF_MONTH, 1);
        dateView.set(Calendar.YEAR, y.intValue());
        final int maxDayOfMonth = dateView.getActualMaximum(Calendar.DAY_OF_MONTH);
        dateView.set(Calendar.DAY_OF_MONTH, Math.min(dayOfMonth, maxDayOfMonth));
        selectedDate = dateView.getTime();
        refreshYearSelector();
                          ...
                          final boolean controlPanel)
  {
    ...
    this.chosenDateButtonColor = UIManager.getColor("textHighlight"); //$NON-NLS-1$
    this.chosenMonthButtonColor = UIManager.getColor("control"); //$NON-NLS-1$
    this.chosenOtherButtonColor = UIManager.getColor("controlShadow"); //$NON-NLS-1$
  ...
  private void refreshYearSelector()
  {
    if (!this.refreshing)
    {
      ...
      final Integer[] years = getYears(this.dateView.get(
          Calendar.YEAR));
      for (int i = 0; i < years.length; i++)
      {
        this.yearSelector.addItem(years[i]);
      }
      this.yearSelector.setSelectedItem(new Integer(this.dateView.get(Calendar.YEAR)));
