...
/**
 * A structural layout element. ReportElements hold style and attributes and allow to establish references that
 * survive cloning via an ID object.
 *
...
{
  /**
   * Assigns a new attribute expression for the given attribute. Attributes are identified by the namespace and
   * name. Each attribute can have only one expression assigned. Setting the expression to <code>null</code> clears
