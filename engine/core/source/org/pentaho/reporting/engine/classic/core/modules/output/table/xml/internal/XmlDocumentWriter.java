  ...
  public void processTableContent( final LogicalPageBox logicalPageBox,
    ...
    if ( tableOpen == false ) {
      ...
      for ( int i = 0; i < columnCount; i++ ) {
        ...
        xmlWriter.writeTag( XmlDocumentWriter.LAYOUT_OUTPUT_NAMESPACE, "column", "width",
          pointShortConverter.format( cellWidth ),
          XmlWriter.CLOSE );
      }
      xmlWriter.writeCloseTag();
    }
    ...
    for ( int row = startRow; row < finishRow; row++ ) {
      ...
      for ( short col = 0; col < columnCount; col++ ) {
        ...
        if ( content == null ) {
          ...
          xmlWriter
            .writeTag( XmlDocumentWriter.LAYOUT_OUTPUT_NAMESPACE, "empty-cell", createCellAttributes( background ),
              XmlWriter.OPEN );
