...
public class PaginationShiftStatePool
{
  ...
  public PaginationShiftState create(final RenderBox box, final PaginationShiftState parent)
  {
    final int nodeType = box.getNodeType();
    if (isBlock(nodeType))
    {
      final BlockLevelPaginationShiftState blockShiftState = blockPool.get();
      blockShiftState.reuse(blockPool, parent, box);
      return blockShiftState;
    }

    final RowLevelPaginationShiftState shiftState = rowPool.get();
