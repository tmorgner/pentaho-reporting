...
public final class ComputeStaticPropertiesProcessStep extends IterateSimpleStructureProcessStep
{
  ...
  private void updateStaticProperties(final RenderBox box)
  {
    ...
    computeComputedWidth(box);
  }

  private void computeComputedWidth(final RenderBox box)
  {
    final RenderBox parent = box.getParent();
    final int type;
    if (parent == null)
    {
      type = LayoutNodeTypes.TYPE_BOX_BLOCK;
    }
    else
    {
    ...
    if ((type & LayoutNodeTypes.MASK_BOX_BLOCK) == LayoutNodeTypes.MASK_BOX_BLOCK)
    {
      ...
      else
      {
        ...
        ((type & LayoutNodeTypes.MASK_BOX_CANVAS) == LayoutNodeTypes.MASK_BOX_CANVAS))
    {
      final long computedWidth = resolveComputedWidth(box);
