...
public class DefaultLayoutModelBuilder implements LayoutModelBuilder, Cloneable
{
  ...
  {
    final StyleSheet resolverStyleSheet = element.getComputedStyle();
    final boolean inline = context.getRenderBox().isAcceptInlineBoxes();
    final RenderBox subReportBox =
        renderNodeFactory.produceSubReportPlaceholder(element, resolverStyleSheet, stateKey, inline);
...
*/
  ...
  /**
   * Used in the report designer to inject the user defined minimum height into the style.
   *
   * @param resolverStyleSheet the resolved style.
   * @return a possibly patched up version of the resolved style.
   */
  ...
  public void startSubFlow(final ReportElement element)
  {
    ...
    if (metaData.isFeatureSupported(OutputProcessorFeature.STRICT_COMPATIBILITY))
    {
      boolean pagebreakBefore = resolverStyleSheet.getBooleanStyleProperty(BandStyleKeys.PAGEBREAK_BEFORE);
      boolean pagebreakAfter = resolverStyleSheet.getBooleanStyleProperty(BandStyleKeys.PAGEBREAK_AFTER);
      final StyleSheet legacyStyle = new SubReportStyleSheet(pagebreakBefore, pagebreakAfter);
      final StyleSheet styleSheet = preProcessSubreportStyle(legacyStyle);
    ...
    {
      final StyleSheet styleSheet = preProcessSubreportStyle(resolverStyleSheet);
      final String layout = computeSubReportStyle();
      box = renderNodeFactory.produceRenderBox(element, styleSheet, layout, stateKey);
    }
  ...
  private String computeSubReportStyle()
  {
    if (context.getRenderBox().isAcceptInlineBoxes())
    {
  ...
  public void addProgressMarkerBox()
  {
    ...
    if (!collapseProgressMarker(child))
    {
  ...
  private void generateProgressMarker()
  {
    if (context.getRenderBox().isAcceptInlineBoxes())
    {
      final InlineProgressMarkerRenderBox markerBox = new InlineProgressMarkerRenderBox();
  ...
  private boolean collapseProgressMarker(final RenderNode child)
  {
    if (isCollapseProgressMarker() && child != null)
    {
      ...
      else if (child.getNodeType() == LayoutNodeTypes.TYPE_BOX_INLINE_PROGRESS_MARKER)
      {
        final InlineProgressMarkerRenderBox markerRenderBox = (InlineProgressMarkerRenderBox) child;
        markerRenderBox.setStateKey(stateKey);
        return true;
      }
    }
