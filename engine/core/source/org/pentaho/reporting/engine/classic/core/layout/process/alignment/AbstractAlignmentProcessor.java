...
//      }
  ...
  {
    final ArrayList<InlineSequenceElement> sequenceElements =
        new ArrayList<InlineSequenceElement>(Arrays.asList(this.sequenceElements));
    final ArrayList<RenderNode> nodes = new ArrayList<RenderNode>(Arrays.asList(this.nodes));

    int sequenceFill = this.sequenceFill;

    boolean changed = false;
    for (int i = 0; i < sequenceFill; i += 1)
    {
      final InlineSequenceElement ise = sequenceElements.get(i);
      final InlineSequenceElement.Classification type = ise.getType();
      if (type == InlineSequenceElement.Classification.CONTENT)
      {
        final RenderNode node = nodes.get(i);
        if (node instanceof SpacerRenderNode)
        {
          sequenceFill -= 1;
          sequenceElements.remove(i);
          nodes.remove(i);
          i -= 1;
          changed = true;
          continue;
      ...
      if (type != InlineSequenceElement.Classification.START)
      {
        if (changed)
        {
          this.sequenceElements = sequenceElements.toArray(this.sequenceElements);
