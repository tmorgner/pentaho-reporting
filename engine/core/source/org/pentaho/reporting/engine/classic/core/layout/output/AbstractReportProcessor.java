...
public abstract class AbstractReportProcessor implements ReportProcessor
{
      ...
      throws ReportProcessingException
  {
    ...
    while (!state.isFinish())
    {
      ...
      {
        final List<Exception> childExceptions = Arrays.asList(errorHandler.getErrors());
        ...
        if (isInRollBackMode)
        {
          ...
          if (nextStateKey.equals(rollbackPageState))
          {
            ...
            if (AbstractReportProcessor.SHOW_ROLLBACKS)
            {
              ...
              {
                AbstractReportProcessor.logger.debug(
                    "Paginate:   (Restore state process key)           : " + restoreState.getProcessKey());
              }
              AbstractReportProcessor.logger.debug(
        ...
        if (pagebreakEncountered == Renderer.LayoutResult.LAYOUT_PAGEBREAK)
        {
              ...
              renderer.isOpen())
          {
                ...
                lastVisibleStateKey.getSequenceCounter() > globalState.getProcessKey().getSequenceCounter())
            {
              ...
              if (AbstractReportProcessor.SHOW_ROLLBACKS)
              {
                logger.debug("Roll-back-key: " + lastVisibleStateKey);
                AbstractReportProcessor.logger.debug(
          ...
          if (onGoingPageBreak)
          {
            ...
            renderer.addEmptyRootLevelBand(state.getProcessKey());
            renderer.endSection();
          }
          ...
          if (state.isFinish() == false)
          {
            ...
            pageStates.add(new PageState(state, outputProcessor.getPageCursor()));
          }
        ...
        if (isInRollBackMode)
        {
          if (nextStateKey.equals(rollbackPageState))
          {
            ...
            if (AbstractReportProcessor.SHOW_ROLLBACKS)
            {
              ...
              {
                AbstractReportProcessor.logger.debug(
                    "Print:                                         : " + restoreState.getProcessKey());
              }
              AbstractReportProcessor.logger.debug(
