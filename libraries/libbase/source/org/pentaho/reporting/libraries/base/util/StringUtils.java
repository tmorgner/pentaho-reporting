...
public class StringUtils
{
   ...
   *
   * @param string    the text to be split.
   * @param separator the separator characters used for the split.
   * @param quote     the quoting character.
   * @return the splitted array.
   */
  public static String[] split(final String string, final String separator, final String quote)
  {
    final CSVTokenizer strtok = new CSVTokenizer(string, separator, quote, false);
    final String[] tokens = new String[strtok.countTokens()];
    int i = 0;
    while (strtok.hasMoreTokens())
    {
  ...
  /**
   * Take a Properties map and convert it to a semicolon separated
   * key/value list where the key and value are separated by an equal
   * sign (i.e. key1=value1;key2=value2;key3=value3
   * @param parameters
   * @return String representation of the Properties
   */
  public static String propertiesToString(final Properties parameters, final String separator)
  {
    final StringBuilder parameterStr = new StringBuilder(100);
  ...
  /**
   * Takes a string that has the format "key=value;key2=value2;..." and converts the string
   * into a Properties map.  The specified deliminator is what is used to tokenize the string.
   * In the above case, the delimnator would be a semi-colon.
   * @param propertiesString
   * @return
   */
  public static Properties stringToProperties(final String propertiesString, final String deliminator)
  {
    Properties properties = new Properties();
    final String [] tokenizedStr = split(propertiesString, deliminator, "'");
    for (int index = 0; index < tokenizedStr.length; index++)
    {
      final String [] keyValuePair = split(tokenizedStr[index], "=");
