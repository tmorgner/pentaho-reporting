...
package org.pentaho.reporting.libraries.base.performance;
...
public class LoggingStopWatch extends StopWatch implements PerformanceLoggingStopWatch
{
  ...
  public void stop(boolean pause)
  {
    ...
    if (message == null)
    {
      logMessage = String.format("start[%d] time[%d] tag[%s] count[%d]", (long) getStartMilliseconds(), (long) getElapsedMilliseconds(), getTag(), getRestartCount());
    }
    else
    {
      logMessage = String.format("start[%d] time[%d] tag[%s] count[%d] message[%s]", (long) getStartMilliseconds(), (long) getElapsedMilliseconds(), getTag(), getRestartCount(), getMessage());
    }
    doLog(logMessage);
    reset();
  }
  ...
  {
    return LogFactory.getLog(LoggingStopWatch.class.getName() + "." + tag).isInfoEnabled();
  }
}
