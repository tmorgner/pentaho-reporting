...
public class BundleMetaDataReadHandler extends AbstractXmlReadHandler
{
  ...
  {
    final BundleMetaDataEntryReadHandlerFactory handlerFactory =
        BundleMetaDataEntryReadHandlerFactory.getInstance();
  ...
  {
    final ArrayList userAttributes = new ArrayList();

    metaData = new DefaultBundleMetaData();
    for (int i = 0; i < entries.size(); i++)
    {
      final BundleMetaDataEntryReadHandler handler = (BundleMetaDataEntryReadHandler) entries.get(i);
    ...
    metaData.putBundleAttribute(ODFMetaAttributeNames.Meta.NAMESPACE,
        ODFMetaAttributeNames.Meta.USER_DEFINED, userAttributes.toArray(new UserDefinedAttribute[userAttributes.size()]));
