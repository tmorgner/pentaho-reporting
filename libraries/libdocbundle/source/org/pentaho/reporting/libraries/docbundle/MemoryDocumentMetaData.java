...
public class MemoryDocumentMetaData implements WriteableDocumentMetaData
{
  ...
  /**
   * Returns the bundle's defined mime-type. This value is read from the "/mimetype" entry (if existent) else from
   * the manifest's "/" entry. The bundle type acts as a hint for the content processor on what content the main
   * document contains. This entry is declarative - if the actual main document does not match the declared bundle
   * type, parsing is allowed to fail.
   *
  ...
  /**
   * Returns the declared mime-type for the given entry. The mime-type is declarative - if it does not match the
   * actual content of the entry, the content processor may raise an error.
   *
  ...
  /**
   * Returns a single document-meta-data attribute. Each attribute is specified by a namespace and attribute name
   * and contains a single string value.
   *
