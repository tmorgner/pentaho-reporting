...
import org.pentaho.reporting.libraries.pensol.vfs.WebSolutionFileObject;
...
public class RepositoryPublishDialog extends RepositoryOpenDialog
{
  ...
  private class NewFolderAction extends AbstractAction
  {
    ...
    public void actionPerformed(final ActionEvent e)
    {
      ...
      SwingUtil.centerDialogInParent(newFolderDialog);
  ...
  private class PublishPasswordValidator extends DocumentChangeHandler
  {
    private PublishPasswordValidator()
    {
    }

    protected void handleChange(final DocumentEvent e)
    {
      getConfirmAction().setEnabled(validateInputs());
    }
  }
  ...
  private KeyedComboBoxModel exportFormatModel;
  ...
  public void init()
  {
    ...
    publishPasswordField.setColumns(25);
    publishPasswordField.getDocument().addDocumentListener(new PublishPasswordValidator());
  ...
  private JPanel createPublishSettingsPanel()
  {
    ...
    if (ReportDesignerBoot.getInstance().getExtendedConfig().getBoolProperty
        ("org.pentaho.reporting.designer.extensions.pentaho.repository.HidePublishPassword") == false)
    {
      c.insets = new Insets(5, 5, 0, 5);
      c.gridwidth = 1;
      c.gridx = 0;
      ...
      c.anchor = GridBagConstraints.WEST;
      c.weightx = 0.0;
      publishSettingsPanel.add(new JLabel(Messages.getInstance().getString("RepositoryPublishDialog.PublishPassword")), c);
      ...
      c.anchor = GridBagConstraints.WEST;
      c.weightx = 0.0;
      publishSettingsPanel.add(publishPasswordField, c);
    }
  ...
  {
    publishPasswordField.setText(password);
  }
  ...
  {
    return (String) exportFormatModel.getSelectedKey();
  }
  ...
  {
    if (ReportDesignerBoot.getInstance().getExtendedConfig().getBoolProperty
        ("org.pentaho.reporting.designer.extensions.pentaho.repository.AllowEmptyPublishPassword") == false)
    {
      if (publishPasswordField.getPassword().length == 0)
      {
        return false;
      }
    }
