...
public class NamedDataSourceDialogModel implements DataSourceDialogModel
{
  ...
  public static class DataSetComboBoxModel extends DefaultComboBoxModel
  {
    public DataSetComboBoxModel()
    {
    }

    public int getIndexForQuery(String name)
    {
      for (int i = 0; i < getSize(); i++)
      {
        final DataSetQuery elementAt = (DataSetQuery) getElementAt(i);
  ...
  {
    queries.addElement(new DataSetQuery(queryName, query, scriptLanguage, script));
  }
  ...
  public void setSelectedQuery(final String selectedQueryName)
  {
    ...
    {
      final DataSetQuery q = (DataSetQuery) queries.getElementAt(i);
