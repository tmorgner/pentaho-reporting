...
public class JdbcDataSourceDialog extends CommonDialog
{
    ...
    private String performQuerySchema(final Connection conn)
    {
      ...
      try
      {
        ...
        if (data.supportsSchemasInTableDefinitions())
        {
          ...
          if (schemas.size() > 1)
          {
            // PRD-3256 - Don't query user for schema unless preferences specify otherwise.
            // Setting the schema in SL restricts users from selecting schema inside SL and
            // the generated SQL does not scope table with schema.
  ...
  private class QueryNameTextFieldDocumentListener extends DocumentChangeHandler implements ListDataListener
  {
    ...
    public void contentsChanged(final ListDataEvent e)
    {
      ...
      try
      {
        ...
        final DataSetQuery selectedQuery = (DataSetQuery) dialogModel.getQueries().getSelectedItem();
                                                   ...
                                                   final String selectedQueryName)
  {
    ...
    if (dataFactory != null)
    {
      ...
      for (int i = 0; i < queryNames.length; i++)
      {
        ...
        DebugLog.log("QueryScript on start: " + script);
      }
  ...
  private SQLReportDataFactory createDataFactory(final JdbcConnectionDefinition connectionDefinition)
  {
    ...
    {
      final DataSetQuery query = (DataSetQuery) queries.getElementAt(i);
      DebugLog.log("QueryScript on end: " + query.getScript());
