...
public final class SaveReportUtilities
{
  ...
  private static final String SAVE_FAILED = ActionMessages.getString("SaveReportUtilities.SaveFailed.Message");
  private static final String OVERWRITE_DIALOG_TITLE = ActionMessages.getString("SaveReportUtilities.OverwriteDialog.Title");
  private static final String FILE_EXTENSION_VERIFICATION_TITLE = ActionMessages.getString("SaveReportUtilities.VerifyFileExtension.Title");
    ...
    if (selectedFile.exists())
    {
      ...
      final int overwrite = JOptionPane.showConfirmDialog(parent,
          ActionMessages.getString("SaveReportUtilities.OverwriteDialog.Message",
              selectedFile.getAbsolutePath()), OVERWRITE_DIALOG_TITLE, JOptionPane.YES_NO_OPTION);
  ...
  public static File validateFileExtension(final File proposedFile, final Component parent)
  {
    ...
    final int result = JOptionPane.showConfirmDialog(parent,
        ActionMessages.getString("SaveReportUtilities.VerifyFileExtension.Message",
            proposedFile.getAbsolutePath()), FILE_EXTENSION_VERIFICATION_TITLE, JOptionPane.YES_NO_CANCEL_OPTION);
                                   ...
                                   final File target)
  {
    ...
    {
      UncaughtExceptionsModel.getInstance().addException(new IOException(SAVE_FAILED, e1));// NON-NLS
