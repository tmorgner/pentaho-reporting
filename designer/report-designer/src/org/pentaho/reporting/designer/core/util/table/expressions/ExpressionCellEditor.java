...
public class ExpressionCellEditor implements TableCellEditor
{
  ...
  private class ExtendedEditorAction extends AbstractAction
  {
    ...
    public void actionPerformed(final ActionEvent e)
    {
      ...
      {
        final Expression originalExpression = (Expression) selectedItem;
        ...
        final Expression expression = optionPane.editExpression(originalExpression, reportDesignerContext);
      ...
      else if (selectedItem instanceof ExpressionMetaData)
      {
        try
        {
          ...
          final Expression resultexpression = optionPane.editExpression(expression, reportDesignerContext);
  ...
  public void init()
  {
    ...
    final ExpressionMetaData[] datas = ExpressionUtil.getInstance().getKnownExpressions();
    Arrays.sort(datas, new PlainMetaDataComparator());
    final DefaultComboBoxModel model = new DefaultComboBoxModel(datas);
                                               ...
                                               final int column)
  {
    ...
    {
      this.expressionEditor.setSelectedItem(value);
    }
    else
    {
      throw new ClassCastException("Expected an expression here.");
    }
    return carrierPanel;
  }
