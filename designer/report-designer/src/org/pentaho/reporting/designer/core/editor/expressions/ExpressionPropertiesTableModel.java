    ...
    extends AbstractTableModel implements ElementMetaDataTableModel, GroupingModel
{
  ...
  protected boolean isFiltered(final ExpressionPropertyMetaData metaData)
  {
    ...
    if (metaData.isExperimental() && WorkspaceSettings.getInstance().isExperimentalFeaturesVisible() == false)
    {
      return true;
    }
    ...
    final Integer compatibilityLevel = activeContext.getMasterReportElement().getCompatibilityLevel();
    if (compatibilityLevel != null)
    {
      if (metaData.getCompatibilityLevel() != -1)
      {
        if (metaData.getCompatibilityLevel() > compatibilityLevel)
        {
          logger.debug("Filtered property " + metaData.getName() +
              " as compatibility-level is: " + metaData.getCompatibilityLevel() + " vs Report: " + compatibilityLevel) ;
          return true;
        }
      }
    }
  ...
  public String getColumnName(final int column)
  {
    ...
    {
      case 0:
        return Messages.getString("ExpressionPropertiesTableModel.NameColumn");
      case 1:
        return Messages.getString("ExpressionPropertiesTableModel.ValueColumn");
      default:
