...
public class ContextMenuUtility
{
  ...
  public static JPopupMenu getCopyOfMenu(final JPopupMenu target, final JPopupMenu source)
  {
    try
    {
      cloneMenuContentes(source, target);
      return target;
    }
    catch (Exception e)
    {
      throw new IllegalStateException("Home-brew Clone failed", e);
    }
  }

  private static void cloneMenuContentes(final JComponent menu, final JComponent copy)
      throws InstantiationException, IllegalAccessException
  {
    ...
    for (int i = 0; i < components.length; i++)
    {
      ...
      {
        final JMenu menuOrg = (JMenu) component;
        final JMenu menuCopy = menuOrg.getClass().newInstance();
        cloneMenuContentes(menuOrg, menuCopy);
        copy.add(menuCopy);
      }
      else if (component instanceof JMenuItem)
      {
        final JMenuItem menuOrg = (JMenuItem) component;
        final JMenuItem menuCopy = menuOrg.getClass().newInstance();
        final Action action = menuOrg.getAction();
      ...
      {
        copy.add(new JPopupMenu.Separator());
      }
      else
      {
        DebugLog.log("Unknown component encountered; " + component); // NON-NLS
      }
    }
  }
